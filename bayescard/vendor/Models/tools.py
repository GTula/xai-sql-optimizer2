...
try:
    import pandas as pd
except ModuleNotFoundError:  # pragma: no cover - fallback para entorno sin pandas
    pd = None
...
def categorical_qcut(series, q, start_value, fanout=0, fanout_values=[]):
    ...
    if pd is None:
        raise ModuleNotFoundError("pandas is required for categorical_qcut")
    bin_freq = 1 / q
...
def discretize_series(series: pd.Series, n_mcv, n_bins, is_continous=False, continuous_bins=None, drop_na=True,
    ...
    """
    if pd is None:
        raise ModuleNotFoundError("pandas is required for discretize_series")
    s = series.copy()
    n_distinct = dict()
    encoding = dict()
    mapping = dict()
